...
function largestTrackHeightForTrackCount(
...
const MiniTracks = ({ heightPx, widthPx }: Props) => {
  const tracks: MIDITrack[] = useSelector((state: GlobalState) =>
    Object.values(state.project.tracks)
  );
  const midiParts: { [id: string]: MIDIPart } = useSelector(
    (state: GlobalState) => state.project.midiParts
  );
  const midiPartInstances: { [id: string]: MIDIPartInstance } = useSelector(
    (state: GlobalState) => state.project.midiPartInstances
  );

  const trackHeight: number = largestTrackHeightForTrackCount(
    heightPx,
    tracks.length
  );

  const projectEndsAtSeconds: number = useSelector(
    projectEndsAtSecondsSelector
  );
  const viewportWidthSeconds: number = useSelector(
    viewportWidthSecondsSelector
  );
  const maxWidthSeconds: number = Math.max(
      ...
      {tracks.map((track: MIDITrack) => {
        ...
        return (
          <Container key={track.id} x={0} y={trackHeight * track.index}>
            {trackMIDIPartInstances.map(
              (trackMIDIPartInstance: MIDIPartInstance) => {
                return (
                  <Rectangle
                    key={trackMIDIPartInstance.id}
                    x={
                      (Transport.toSeconds(trackMIDIPartInstance.time) /
                        maxWidthSeconds) *
                      widthPx
                    }
                    y={0}
